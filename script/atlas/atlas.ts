class Atlas {
	...
	private props: OffscreenCanvas;
	private objects: OffscreenCanvas;
	private sprites: OffscreenCanvas;
	private dynamicSprites: OffscreenCanvas;
	private itemSprites: OffscreenCanvas;
	private tileCtx: OffscreenCanvasRenderingContext2D;
	private propCtx: OffscreenCanvasRenderingContext2D;
	private objectCtx: OffscreenCanvasRenderingContext2D;
	private spriteCtx: OffscreenCanvasRenderingContext2D;
	private dynamicSpriteCtx: OffscreenCanvasRenderingContext2D;
	private itemSpriteCtx: OffscreenCanvasRenderingContext2D;
	private renderVersion: number = 0;

	constructor(tileSize: number, texturesPerRow: number) {
		...
		this.objects = new OffscreenCanvas(
			spriteSize * this.texturesPerRow,
			spriteSize * Math.ceil(dynamicObjects.length / this.texturesPerRow),
		);
		...
		this.itemSprites = new OffscreenCanvas(
			spriteSize * this.texturesPerRow,
			spriteSize * Math.ceil(itemManager.getAllItems().length / this.texturesPerRow),
		);
	...
	getTexturesPerRow(): number {
		return this.texturesPerRow;
	}

	getTileAtlas(): OffscreenCanvas {
		return this.tiles;
	}

	getPropAtlas(): OffscreenCanvas {
		return this.props;
	}

	getObjectAtlas(): OffscreenCanvas {
		return this.objects;
	}

	getSpriteAtlas(): OffscreenCanvas {
		return this.sprites;
	}

	getDynamicSpriteAtlas(): OffscreenCanvas {
	...
	loadTextures() {
		...
		for (let i = 0; i < props.length; i++) {
			if (i === 0) continue; // Skip the default empty prop
			const prop = props[i];
			const img = new Image();
			img.onload = () => {
				const x = (i % this.texturesPerRow) * this.getSpriteSize();
		...
		for (let i = 0; i < dynamicObjects.length; i++) {
			const object: DynamicObject = dynamicObjects[i];
	...
	async drawDynamicSprite(dynamicCreature: DynamicCreature): Promise<void> {
		const currentRender: number = ++dynamicCreature.renderVersion; // Increment render version for each draw call
		...
		const texturesToRender: DynamicSpriteTextures = dynamicCreature.getTexturesToRender();
		...
		const images = await Promise.all(
			loadOrder.map(async (texturePath: string) => {
				const img = new Image();
				img.src = texturePath;
		...
		items.forEach((eq) => {
			...
			if (item instanceof Armor) {
				const equippableData = item.getEquippableItemData();
				equippableData.forEach((data: EquippableItemData) => {
					...
					ctx.drawImage(
						this.itemSprites,
						data.texturePosition?.x || 0,
						data.texturePosition?.y || 0,
						this.getSpriteSize(),
						this.getSpriteSize(),
						x + anchor.x - width / 2,
						y + anchor.y - height / 2,
	...
	getDynamicSpriteTexturePosition(dynamicCreature: DynamicCreature): { x: number; y: number } {
