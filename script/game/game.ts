...
class Game {
	...
	private mouseHeldDownStart: number = 0; // Timestamp when the mouse was first held down, used to differentiate between clicks and holds
	private keysPressed: Set<string> = new Set();
	private fpsCounter: HTMLDivElement;
	private frameCount: number[] = [];
	private mouseX: number = 0;
	private mouseY: number = 0;
	private devInfo: HTMLDivElement = document.querySelector(".dev-info") as HTMLDivElement;
	private paused: boolean = false;
	private pausedOverlay: HTMLDivElement = document.querySelector(".paused") as HTMLDivElement;
	private controlledCreatureId: string | null = null;
	private animationSpeedMultiplier: number = 1;
	private pathRequestQueue: {
		start: { x: number; y: number };
		goal: { x: number; y: number }[];
		creature: Creature;
		forHighlight: boolean;
	}[] = [];
	private PATH_REQUEST_BUDGET_PER_FRAME: number = 4; // How many ms to spend on processing path requests each frame
	private isEditorMode: boolean = false; // Flag to indicate if we're in editor mode, which can disable certain game mechanics for testing
	private keyHeldInterval: number = 75; // Interval in ms for processing held keys
	private lastKeyHeldProcessed: number = 0; // Timestamp of the last time we processed held keys

	constructor() {
		this.fpsCounter = document.querySelector(".fps-counter") as HTMLDivElement;
	...
	setControlledCreatureId(id: string | null) {
	...
	getControlledCreatureId(): string | null {
	...
	mouseHeldDownDuration(): number {
		if (!this.mouseHeldDown) return 0;
		return performance.now() - this.mouseHeldDownStart;
	}

	isMouseHeldDown(): boolean {
	...
	loop(): void {
		...
		const dt: number = (this.time - this.lastTime) / 1000;
	...
	requestPath(start: { x: number; y: number }, goal: { x: number; y: number }[], creature: Creature, forHighlight: boolean = false): void {
		if (this.isInEditorMode()) return; // Don't allow pathfinding requests in editor mode, as it can interfere with testing and cause unintended consequences
	...
	updateCreatureVisualPositions(dt: number) {
		...
		for (const creature of entityManager.getCreaturesOnMap(map.id)) {
			...
			creature.setScreenPosition(
				this.approach(currentScreenPos.x, targetScreenX, this.getAnimationSpeed() * dt),
				this.approach(currentScreenPos.y, targetScreenY, this.getAnimationSpeed() * dt),
			);
	...
	update(dt: number) {
		...
		if (this.keysPressed.has("w") || this.keysPressed.has("ArrowUp")) inputY -= 1;
		if (this.keysPressed.has("s") || this.keysPressed.has("ArrowDown")) inputY += 1;
		if (this.keysPressed.has("a") || this.keysPressed.has("ArrowLeft")) inputX -= 1;
		if (this.keysPressed.has("d") || this.keysPressed.has("ArrowRight")) inputX += 1;
		...
		if (inputX === 0) camera.decelerateX(dt);
		if (inputY === 0) camera.decelerateY(dt);
	...
	snapCameraToZoom() {
		const centreX: number = this.mouseX;
		const centreY: number = this.mouseY;
		const worldX: number = camera.getX() + centreX / camera.getPrevZoom();
		const worldY: number = camera.getY() + centreY / camera.getPrevZoom();
	...
	updateDevInfo() {
		...
		const hoveredObject: DynamicObject = mapRenderer.getMap().getBoundingObjectsAt(x, y)[0];
		const prop: Prop = mapRenderer.getHighlightedTilePropData();
		const displayProp: boolean = prop && prop.getId() !== "empty";
	...
	checkHotkeys(key: string) {
		if (this.isInEditorMode()) return; // Disable hotkeys in editor mode to prevent conflicts with editor controls
		if (key === " ") {
			this.togglePause();
		}
		if (key === "r") {
	...
	updateMovingCreatures(dt: number) {
		const map = mapRenderer.getMap();
		if (!map) return;
		for (const creature of entityManager.getCreaturesOnMap(map.id)) {
	...
	selectTile() {
		...
		if (highlightedTile.x === -1 || highlightedTile.y === -1) return;
