...
const getDynamicObjectById = (id: string): DynamicObject | undefined => {
	let obj: DynamicObject = dynamicObjects.find((obj) => obj.getId() === id);
	if (!obj) {
		return undefined;
	}
	return new DynamicObject(obj as any); // Return a new instance to avoid shared state issues
};

const getTexturePositionForDynamicObject = (obj: DynamicObject): { x: number; y: number } => {
