...
class CreatureCombat {
    ...
    opportunityAttacksLeft; // This is used to track the number of opportunity attacks a creature can still make in a turn, which can be modified by feats, buffs, etc.
    movement = 0;
    ...
    async provokeOpportunityAttacks(hostiles) {
        ...
        await Promise.all(nearbyHostiles.map(async (hostile) => {
    ...
    iterateAttacks(attackType, iterations) {
        ...
        for (let i = 0; i < iterations; i++) {
            if (!ctx.weapon)
                break; // If there is no weapon equipped in the relevant slot, stop iterating
            let penalty = 0;
    ...
    getWeaponAttackBonus(ctx) {
        ...
        // Ability modifier
        let abilityModifier = abilityModifiers.strength; // Default to strength for melee attacks
    ...
    async performAttackSequence(weaponIndex, iterations, target, tile, iterationType) {
        const ctx = this.owner.inventory.getEquippedWeapons()[weaponIndex];
        for (let i = 0; i < iterations; i++) {
    ...
    async makeOpportunityAttack(target) {
