    ...
    renderCreatures(camera) {
        ...
        for (const creature of entityManager.getCreaturesOnMap(this.map.id)) {
            if (!creature.stats.isAlive())
                continue; // Skip rendering dead creatures
...
const addPlayerPartyMember = (id, index, bodyType) => {
...
const devInit = () => {
    ...
    const creature2 = entityManager.addCreature(testPlayer2, mapRenderer.getMap().id, -1, -1, "dev_testing_area:spawn_point:5:3");
